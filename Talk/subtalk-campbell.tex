...
\begin{center}
New: Actual prediction physics at scale \prj{\tiny}{M.~Anderson}
\end{center}
  \begin{itemize}
  \item Dedicated Application Time (DAT) last Fall
  \item Large portion of Lassen:
    \begin{itemize}
    \item 600(ish) available nodes
...
\begin{itemize}
\item \mirgecom{} has prediction-supporting performance (poised to deliver more)
\item Next focus: understanding and improving \mirgecom{} performance
\end{itemize}
\begin{center}
Next steps
\end{center}
...
\item Auto-tuning, flop monitoring \prj{\tiny}{Nick Christensen}
\item DAG Splat \prj{\tiny}{M. Smith, Kl\"ockner}
% \item Performance:
%\begin{itemize}
\item More platforms: Tioga, Delta, ...
\item Modeling and benchmarks (bake-off)
%\end{itemize}
\end{itemize}
\columnbreak
\item Imminent enhancements:
\begin{itemize}
\item More verification: mixture and boundary conditions
\item Workflow management: \textit{Parsl} \prj{\tiny}{D.~Friedel}
  \begin{itemize}
  \item For interplatform testing \& timing
  \item UQ \prj{\tiny}{K.~Tang}
  \item Prediction
  \end{itemize}
\item Tensor product elements (Hexes) \prj{\tiny}{Addison Alvey-Blanco, M.~Anderson, T.~Ricciardi}
